...
module.exports = class BaseJavascriptModule {
    ...
    async _completeLoading(deviceClass) {
        ...
        await Promise.all((this.manifest.child_types || []).map(async (childId) => {
            ...
            const childManifest = await this._loader.loadManifest(childId, true);
            const submodule = await this._createSubmodule(childId, childManifest, subdevices[childId]);
    ...
    getDeviceFactory() {
        ...
        return this._loading = this._doGetDeviceFactory();
    }
};
