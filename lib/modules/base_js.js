...
const { makeGenericOAuth, makeBaseDeviceMetadata, getPollInterval } = require('./utils');
...
module.exports = class BaseJavascriptModule {
    ...
    async _completeLoading(deviceClass) {
        ...
        if (config.module === 'org.thingpedia.config.oauth2' && !deviceClass.runOAuth2)
            makeGenericOAuth(this._id, config, deviceClass);
