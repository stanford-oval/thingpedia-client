...
function get(obj, propchain) {
    // split the prop chain by "." except the ones with double escape backslash
    for (let prop of propchain.match(/(\\.|[^.])+/g))
        // remove double backslash if there is any
        obj = obj[prop.replace('\\', '')];
    return obj;
}
